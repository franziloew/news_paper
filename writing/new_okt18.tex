...
% -- Bilden online Nachrichten die politische Realität ab? -> 3 types of bias
% ------------------------------
...
\todo{Say some words on unsupervised method (no labeled data)}
...
%---- 2. Measure sentiment in digital news ---%
...
The discussion about the influence of digital media on the political opinion-forming process has gained momentum since the presidential elections in October 2016. In recent years, the importance of the Internet as a source of information for political topics has been growing strongly. Even though television remains the most widely used source of news in Germany (2018: 74\%), numbers watching continue to decline while use of the internet for news has grown significantly in the last year (+5\%, 2018: 65\%) \citep{holig_reuters_2018}. The expansion of the internet as a new method of communication provides a potential challenge to the primacy of the traditional media and political parties as formers of public opinion \citep{savigny_public_2002}.

Schon vor dem Internet war die Rolle der (Massen)medien ein ongoing Untersuchungsgegenstand der social science. Die vorherrschende These dabei ist, dass die Medien als Hauptinformationsquelle für die Wähler ein wichtiger Faktor bei die Meinungsbildung sind \citep{ferree_four_2002, mccombs_look_2005, eberl_one_2017}. One central responsibility of the media arguably is to supply voters with balanced and objective information on relevant political issues and actors \citep{stromback_four_2008, eberl_one_2017}. Bias in political news coverage may have a profound influence on voter opinions and preferences \citep{eberl_one_2017}. So kann beispielsweise eine stärkere Verbreitung von politischen Inhalten einer Partei einen positiven Einfluss auf die Einstellung gegenüber dieser Partei haben \citep{benewick_floating_1969}. Vor der Annahme, dass Medien einen Einfluss auf Wählerpräferenzen haben, werfen \citet{mccombs_agenda-setting_1972} die Frage auf, ob die politische Welt in den Medien unverzerrt abgebildet wird. Sie kommen zu dem Ergebnis, dass Massenmedien die politische Agenda beeinflusst. 
...
In studying the effects of media bias, three sub-types of biases are commonly identified in the literature \citep{dalessio_media_2000, eberl_one_2017, junque_de_fortuny_media_2012}: (1) Coverage bias, (2) statement bias and (3) gatekeeping bias.\footnote{\citep{eberl_one_2017} refer to the as (1) visibility bias, (2) tonality bias and (3) agenda bias.} Those three concepts measure how often political actors appear in the media (visibility bias), how they are evaluated (tonality bias) and whether political actors are allowed to present their own policy positions and talk about their issues in the media (gatekeeping bias). The gatekeeping bias therefore stems from a journalist's or editor's decision to select or ignore specific news stories. Most of the literature on media bias focuses on one type of bias and most research tends to disregard agenda bias as the operationalization is somewhat more challenging. In order to know which news stories have been selected as well as deselected by journalists, one would have to know the universe of news stories at a given point in time \citet{dalessio_media_2000}.  Ich übernehme die Idee von \citet{eberl_one_2017} in using parties' campaign communication as an approximation of the potential universe of news stories. However, my analysis differs from earlier approaches in that I use machine learning techniques to identify the underlying topics in the text corpus.

To discover the latent topics in the corpus of text data, the structural topic modeling (STM) developed by \citet{roberts_model_2016} is applied. The STM is an unsupervised machine learning approach that models topics as multinomial distributions of words and documents (as a synonym for news articles) as multinomial distributions of topics, allowing the incorporation of external variables that affect both, topical content and topical prevalence. The results of the generative process of the STM are two posterior distributions: One for the topic prevalence in a document (what is the article or press release about?) and one for the content of a topic (what is the topic about?).

Building on the insight that media bias can have three different forms, this study makes two key contributions. (1) combination of different text-mining techniques to analyze a large text corpus reduces the human induced bias and makes research nachvollviehbarer und vergleichbarer, (2) technique to measure agenda bias.    
 

After assigning a topic to each article, a dictionary-based sentiment analysis is conducted to estimate how topics are discussed differently by different news websites. To conduct such an analysis, a list of words (dictionary) associated with a given emotion, such as negativity is pre-defined. The document is then deconstructed into individual words and each word is assigned a sentiment value according to the dictionary, where the sum of all values results in the emotional score for the given document.   
