...
Ideas
\begin{enumerate}
	\item Estimate the sentiment values of each topic, for each news website. \citep{naskar_sentiment_2016}
...
\citet{benewick_floating_1969}: Eine st‰rkere Verbreitung von politischen Inhalten einer Partei ist positiv mit der Einstellung gegen¸ber dieser Partei verbunden war. 

\citet{mccombs_agenda-setting_1972} Massenmedien kˆnnten die Agenda der politischen Kampagnen bestimmen. Sie werfen die Frage auf, ob die Massenmedien die politische Welt tats‰chlich perfekt abbilden. In einem unvollkommenen Umfeld kˆnnten sich Verzerrungen der Medien in der ˆffentlichen Meinung ausbreiten und somit die Bevorzugung gegen¸ber der einen oder anderen Partei oder Politikerin beeinflussen und so die politische Realit‰t pr‰gen. Die meisten dieser Verzerrungen werden durch den Bearbeitungs- und Auswahlprozess eingef¸hrt. 

Im digitalen Zeitalter hat das Internet seinen Platz als Massenmedium neben dem Fernsehen eingenommen (Fredricksen, 2010) und die genannten Anliegen werden auch f¸r das Internet zunehmend relevanter.

In einer Meta-Analyse unter Ber¸cksichtigung von 59 Studien (mit Daten ¸ber parteiische Medienvorurteile im Pr‰sidentschaftswahlkampf seit 1948) fanden \citet{dalessio_media_2000} drei vorherrschenden bias metrics, die zur Untersuchung parteiischer Medienvoreingenommenheit verwendet wurden.
...
\begin{enumerate}
	...
	\item coverage bias: Misst die physische Deckungssumme, die jede Seite der Ausgabe erh‰lt. Traditionell wird dies in Spaltenzoll, Anzahl der Schlagzeilen oder in der Sendezeit, die den Seiten der Ausgabe gewidmet ist, gemessen. \citet{junque_de_fortuny_media_2012} messen die Reichweite als die Anzahl der Online-Artikel. In politischen Fragen kann jede Partei oder jeder Politiker als eine "Seite" oder eine Einheit betrachtet werden. Sie argumentieren, dass die faire Deckung durch eine a priori-Verteilung bestimmt wird. Diese a priori-Verteilung repr‰sentiert alle Einheiten der Bevˆlkerung nach ihrer relativen Bedeutung, gemessen an den Wahlstimmen der letzten Wahlen. Groﬂe Abweichungen von der fairen Verteilung zeigen tendenziell eine gewisse Verzerrung in Richtung der einen oder anderen Einheit. The coverage c(e,s) of an entity e by a newspaper s is defined as the number of news articles published by the newspaper on that entity, normalised on the total amount of articles by that newspa- per in the corpus.
	\item statement bias: Unterscheidung zwischen g¸nstigen und ung¸nstigen oder positiven und negativen Nachrichten. \citet{junque_de_fortuny_media_2012} messen diese verzerrung mit einer sentiment analyse. Ein Wort der Vorsicht ist angebracht, wenn es um die Interpretation dieser'Gef¸hle' in Nachrichtenartikeln geht. Wenn ein Artikel ¸ber eine Entit‰t als negativ eingestuft wird, bedeutet dies nicht, dass eine Nachrichtenquelle oder ein Journalist diese Entit‰t benachteiligt. Es kˆnnte sein, dass die Entit‰t absichtlich negative Kritik in den Artikel einbringt. Diese Maﬂnahme ist daher als assoziative Maﬂnahme zu verstehen (d.h. eine Partei wird mit einem negativen Image assoziiert, wenn der grˆﬂte Teil ihrer Berichterstattung negative Inhalte enth‰lt). Es kˆnnen keine R¸ckschl¸sse darauf gezogen werden, ob dieses Bild von dem betreffenden Unternehmen oder von der Nachrichtenquelle stammt, d.h. wir kˆnnen nur beweisen, dass es existiert.
\end{enumerate}
...
\input{tables/wordcount}\label{t_textlength}
...
It becomes apparent that topic 4 about the coalition talks between CDU/CSU and SPD - the "Grand coalition" or "GroKo" - is the topic with the highest expected frequency in the whole corpus, followed by the topic about the so-called Jamaica parties (CDU/CSU, FDP and B90/Die Gr√ºnen), which was the first alternative to be negotiated directly after the elections.  
...
\begin{enumerate}
	...
	\item Topic 2: About B90/Die Gr√ºnen, mainly covering issues regarding the party's personell debates.
	...
	\item Topic 23: About issues regarding the CSU, mainly about the competition between Horst Seehofer and Markus S√∂der and the negotiations with the CDU/CSU.
	...
	\item Topic 27: Covering the Jamaica coalition talks, mainly focusing on the smaller players B√ºndnis B90/Die Gr√ºnen and FDP.	
	\item Topic 30: About the AfD, mainly about the resignation of Frauke Petry and J√∂rg Meuthen.
