...
corr.df %>%
  ...
  coord_flip() +
  theme_hc() +
  labs(x=NULL, y=NULL, title="Weighted topic probability", subtitle = "Using SentiWS2") +
  facet_grid(.~source)
```
...
corr.df.dates <- theta %>%
  ...
  dplyr::summarise(
    ...
    SentiWS_theta = mean(SentiWS_theta2, na.rm = T),
    SentiWS = mean(SentiWS_polarity, na.rm = T)
  ) %>%
  ungroup() 
