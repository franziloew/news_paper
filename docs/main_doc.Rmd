...
# 3. combine both
# ---------------
# sentDict <- bind_rows(SentiWS, BPW) %>% distinct(word, .keep_all = T)
...
```{r}
sentTxt_token %<>% 
  mutate(sent = ifelse(polarity >= 0, "positive", "negative")) 
```
...
sentTxt_token %>% 
  ...
  ggthemes::theme_hc() +
  labs(x = NULL, y = "Frequency", title="News articles") +
  coord_flip()
```
...
plotSentParty <- function(dat) {
  dat %>% 
    ...
    geom_col(show.legend = FALSE, fill=color1) +
...
sentTxt_token %>% 
  filter(type=="news") %>%
  filter(sent == "positive") %>%
  select(type, source, word, polarity, sent) %>%
  count(word, sent, source, sort = TRUE) %>%
  ungroup %>%
  group_by(source) %>%
  top_n(10, n) %>%
...
model_df <- left_join(model_df, sentTxt, by="doc_index")

save(sentTxt, file = "../output/sentTxt.Rda")
```

```{r}
model_df %>% 
  group_by(source, type) %>% 
  summarise(meanSent = mean(polarity, na.rm = T),
            meanSent_dummy = mean(sent_dummy, na.rm = T)) %>%
  ggplot() +
  geom_point(aes(reorder(source,meanSent), scaledX(meanSent))) +
  geom_point(aes(reorder(source,meanSent), scaledX(meanSent_dummy)), color = color2) +
  geom_hline(yintercept = 0, color = "red", linetype = 2) +
  coord_flip() +
  ggthemes::theme_hc() +
  labs(x=NULL, y="Mean sentiment") +
...
# 3. Combination
sentDict.token <- sent_token %>% left_join(., sentDict, by = "word" )
```

```{r Calculacte document sentiment-socres, eval=FALSE, include=FALSE}
...
sent_df3 <- sentDict.token %>% 
  group_by(doc_index) %>% 
  dplyr::summarise(
                  n = n(),
                   sentDict_dummy = sum(senti_dummy, na.rm = T),
                   sentDict = sum(senti_dummy, na.rm = T)/n
                   ) %>%
  select(-n)

model_df <- left_join(model_df, sent_df1, by="doc_index") %>% 
  left_join(., sent_df2, by="doc_index") %>% 
  left_join(., sent_df3, by="doc_index")
```

```{r eval=FALSE, include=FALSE}
model_df %>%
  transmute(title =title, 
            SentiWS =round(SentiWS, 3), 
            BPW =round(BPW, 3), 
            sentDict=round(sentDict, 3)) %>%
  sample_n(10) %>% htmlTable::htmlTable()
```
...
sentiment_df <- BPW %>% 
  select(word, senti_dummy) %>% 
  transmute(
    words = word,
    polarity = as.numeric(senti_dummy)
  ) %>%
...
sent_df <- model_df %>% 
  select(doc_index, source, type, n, SentiWS_polarity, SentiWS_dummy, SentiWS,
         BPW_dummy, BPW, sentDict_dummy, sentDict, sentimentR)

save(sent_df,sentimenR.df, sentDict.token, 
     SentiWS.token, BPW.token,  file="../output/sentiment.Rda")
```
...
model_df %<>% filter(type=="news")

processed <- textProcessor(model_df$text_cleaned, 
                           metadata = model_df[,c("source","text_cleaned")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
...
tidy_news_df <- model_df %>%
  select(source, type, text_cleaned, doc_index) %>%
...
covariates <- tidy_news_df %>%
    sample_frac() %>%
    distinct(doc_index, source, type)
```
...
stmOut <- stm(documents = out$documents, vocab = out$vocab,
              ...
              max.em.its = 75, data = out$meta, verbose = F,
              init.type = "Spectral")

save(model_df, out, stmOut, file = "../output/models/finalmodel_50_nocontent_nopress.RDa")
```
...
# sentiment results
load("../output/sentTxt.Rda")
k <- stmOut$settings$dim$K
  
model_df <- model_df %>%
  dplyr::mutate(doc_index = as.numeric(rownames(.))) %>%
  left_join(., sentTxt, by="doc_index")
...
```{r}
td_gamma <- tidy(stmOut, matrix = "gamma",
...
```{r For STM Model without content prior}
# DONT RUN
topics.df <- as.data.frame(sagelabs$cov.betas[[1]]$problabels) %>%  
  transmute(topic = as.numeric(rownames(.)),
            label1 = paste(V1,V2,V3,V4,V5,V6,V7))
...
```{r eval=FALSE, include=FALSE}
sample[1,] %>%
  select(polarity, sent_dummy) %>%
  gather(metric, senti) %>%
  ggplot(aes(senti, metric, label=round(senti, 3))) +
  geom_point(aes(color = metric), size=3) +
  geom_text(vjust=-1) +
  geom_vline(xintercept = 0, linetype=2 ) +
  theme_hc() +
  ggthemes::scale_color_gdocs() +
  labs(x=NULL, y=NULL, title = "Sentiment scores of sample Document")
```
...
```{r Plot expected frequency, fig.height=6, fig.width=8}
topicmean %>%
...
$$
\text{NTS}^k_d=\theta^{k}_{d}*\text{NT}_d*(\frac{1}{n_d})
$$

where $\theta^k_{d}$ is the proportion of topic $k$ in document $d$ obtained from the STM procedure, $NT_d$ is the number of positive words minus the number of negative words (nettone) in $d$ and $n$ is the number of total words in document $d$.

```{r}
theta <- theta %>% 
  mutate(polarity_theta = theta*polarity_raw*(1/n),
         dummy_theta = theta*sent_dummy_raw*(1/n))
```
...
plot <- theta %>%
  group_by(topic,joint_label) %>%
  dplyr::summarise(
    polarity_theta = mean(polarity_theta, na.rm = T),
    dummy_theta = mean(dummy_theta, na.rm = T)
  ) %>%
  ungroup() %>%
   gather(metric, sent_value, polarity_theta:dummy_theta) %>%
   group_by(metric) %>%
  mutate(
    sent_m = median(sent_value)
  ) %>% ungroup() 
...
### Assign the topics to a party

```{r}
cdu <- paste(c("cdu","csu","union","merkel"),collapse = "|")
spd <- paste(c("spd","schulz"),collapse = "|")
...
```{r}
party_topics_df<-as.data.frame(sagelabs$cov.betas[[1]]$scorelabels, stringsAsFactors = F) %>% 
  mutate(topic=as.numeric(rownames(.))) %>% 
  group_by(topic) %>%
  summarise(label = paste(V1,V2,V3,V4,V5,
                               collapse = ", "),
            # take the 10 most likely words to check the content
            content = paste(V1,V2,V3,V4,V5,
                            V6,V7,V8,V9,V10, collapse = ", ")) %>%
  ungroup() %>% 
  mutate(cdu = ifelse(grepl(cdu, content, ignore.case = T),1,NA),
         spd = ifelse(grepl(spd, content, ignore.case = T),1,NA),
         fdp = ifelse(grepl(fdp, content, ignore.case = T),1,NA),
         afd = ifelse(grepl(afd, content, ignore.case = T),1,NA),
         gruene = ifelse(grepl(gruene, content, ignore.case = T),1,NA),
         linke = ifelse(grepl(linke, content, ignore.case = T),1,NA),
         label = paste0(as.character(topic),": ", label)
         )
# plot
party_topics_df %>%
...
How do media outlets report about party-related topics?

```{r}
df.new <- theta %>% 
  left_join(., party_topics_df, by="topic") %>% 
  group_by(doc_index, source, type, joint_label, cdu, spd, fdp, afd, gruene, linke) %>%
  summarise(sent_theta1 = mean(polarity_theta, na.rm = T),
            sent_theta2 = mean(dummy_theta, na.rm = T)) %>%
  ungroup() %>%
  group_by(joint_label, source, type, cdu, spd, fdp, afd, gruene, linke) %>%
  summarise(
    count = n(),
    sent_theta1 = mean(sent_theta1),
    sent_theta2 = mean(sent_theta2)) %>%
  ungroup() %>%
  gather(party, ident, cdu:linke)
```

What is the topic with the highest weighted topic prob. for each source?

```{r}
df.new %>%
  filter(!is.na(ident)) %>%
  group_by(source, type, joint_label) %>%
  summarise(sent_theta2 = mean(sent_theta2, na.rm = T)) %>%
  top_n(1, sent_theta2) %>%
  ggplot(aes(reorder(source, sent_theta2), sent_theta2)) +
  geom_col(aes(fill = sent_theta2), show.legend = F) +
  geom_text(aes(label = joint_label),
            hjust = 0, nudge_y = 0.0005, size = 3,
            family = "IBMPlexSans") +
  coord_flip() +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0, 0.02)) +
  ...
  labs(x = NULL, y = expression(gamma),
       title = "Top topics by weighted topic probability",
       subtitle = "With the top words that contribute to each topic")
```
...
p1 <- df.new %>%
  ...
  filter(type == "press") %>%
  filter(!is.na(ident)) %>%
  group_by(joint_label, source) %>%
  summarise(sent_theta = mean(sent_theta2, na.rm = T)) %>%
  ggplot(aes(source, sent_theta*1000)) +
  geom_col(aes(fill=sent_theta), show.legend = F) +
  #geom_text(aes(label=source), size=2) +
  facet_wrap(~joint_label, ncol = 1) +
  coord_flip() +
  labs(x=NULL, y=NULL, title="Press releases") +
  theme_hc() 
...
p2 <- df.new %>%
  ...
  facet_wrap(~joint_label, ncol = 1) +
  coord_flip() +
  labs(x=NULL, y=NULL, title="News articles") +
  theme_hc() 
...
plot <- corr.df.dates %>%
  ...
  ) %>% gather(metric, val, theta:polarity_theta) 
...
```{r fig.align="centre", message=FALSE, warning=FALSE}
corr.df.sm <- corr.df %>%
