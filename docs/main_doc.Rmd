...
NTm is defined as the number of all positive words minus negative words divided by the total number of words of each statement m.

The sentiment score $SC_d$ for each document $d$ is defined as the number of all positive words minus negative wirds devided by the total number of words of that document:
 
$$
\text{SentScore}_d = \frac{|\text{positive words}_d| - |\text{negative words}_d|}{|\text{TotalWords}_d|}
$$
...
```{r Sample sentiment analysis 1 - calculate score}
SentiWS.token %>% filter(doc_index == doc) -> sample
mean(sample$senti_dummy, na.rm = T)
```
...
```{r Sample sentiment analysis 2 - calculate score}
...
```{r Sample sentiment analysis 3}
sentDict.token %>% filter(doc_index == doc) -> sample

sum(sample$senti_dummy, na.rm = T)/nrow(sample)
```
...
What is the sentiment value of that topic?

```{r}
sample[1,] %>%
  select(SentiWS, BPW, sentDict, sentimentR) %>%
  gather(metric, senti) %>%
...
```{r Plot expected frequency, fig.height=6, fig.width=5}
topicmean %>%
...
$$
\text{NTS}^k_d=\theta^{k}_{d}*\text{NT}_d*(\frac{1}{L_d})
$$

where $\theta^k_{d}$ is the proportion of topic $k$ in document $d$ obtained from the STM procedure and $NT_d$ is the number of positive words minus the number of negative words (nettone) in $d$ and $L_d$ is the number of words in document $d$.
...
### Agenda congurence (Duncan segregation index)

Congruence between two agendas can be calculated for every pair of media outlet and party. The formula reads the following (Duncan and Duncan, 1955)

$$
\text{congruence}=100-\frac{\sum_{i=1}^n |\text{NTS media}^k-\text{NTS party}^k|}{2}
$$

Where $\text{NTS media}^k$ is the $\text{NTS}$ of topic $k$ of a media outlet aggregated over all documents (news articles) belonging to that outlet. Similary $\text{NTS party}^k$ is the $\text{NTS}$ of topic $k$ of a party aggregated over all documents (press releases) belonging to that party.
...
Rescale the dimap values [1,11] on the reuters range [1,7]

```{r Prepare and combine data sources}
# Prepare dimap data
...
3. ... compare with topic correlations
...
```{r}
# prepare congruency data
cong_p <- cong %>%
  mutate(medium = rownames(.)) %>%
  gather(party, congruency, parties) 

final_df <- left_join(final_df, cong_p, by = c("medium","party")) %>%
  mutate(congru_rescaled = rescale(congruency, min(congruency), max(congruency),0,1))
```

```{r}
p1 <- final_df %>%
  select(-count_relative,  corr, -congruency) %>%
  gather(metric, value, count_rel_scaled:congru_rescaled) %>%
  ...
  scale_y_discrete(labels=c("Congruency","Topic correlation","Reader preferences")) +
  theme(
...
p2 <- final_df %>%
  group_by(medium) %>%
  dplyr::summarise(corr_corr = cor(count_relative, corr)) %>%
  ggplot(aes(medium, corr_corr)) +
  geom_col(aes(fill=corr_corr), show.legend = F) +
  geom_text(aes(label=round(corr_corr,2)), vjust=-0.1, color="red", size= 3) +
  geom_hline(aes(yintercept=0)) +
  theme_void() +
  labs(caption="Corr/Corr") +
...
p3 <- final_df %>%
  group_by(medium) %>%
  dplyr::summarise(corr_cong = cor(count_relative, congru_rescaled)) %>%
  ggplot(aes(medium, corr_cong)) +
  geom_col(aes(fill=corr_cong), show.legend = F) +
  geom_text(aes(label=round(corr_cong,2)), vjust=-0.1, color="red", size= 3) +
  geom_hline(aes(yintercept=0)) +
  theme_void() +
  labs(caption="Corr/Cong") +
  scale_y_continuous(limits = c(-1,1), position = "right") +
  facet_wrap(medium~., scales = "free_x", ncol = 1) +
  theme(strip.text = element_blank())

p1 + p2 + p3 + plot_layout(width = c(10,1,1))
```
