...
```{r, include=FALSE}
# load the packages
libs <- c("readr","lubridate","tidyverse","patchwork","data.table","stringr","scales","ggthemes","xtable")
...
load("../output/btw_combined_new.Rda")
```
...
ggsave(filename = "../figs/visits.png", width = 6)
```
...
ggsave(filename = "../figs/article_timeline.png")
```

```{r}
btw %>%
  ggplot(aes(medium, fill = medium)) +
...
ggsave(filename = "../figs/article_distribution.png", width = 4)
```
