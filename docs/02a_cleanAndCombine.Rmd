...
df_full <- dbGetQuery(con, 'SELECT * FROM news_full')
```
...
df_full %>%
  ...
  ggthemes::theme_fivethirtyeight() +
  facet_wrap(~ site) +
  labs(title = "Number of articles/day", subtitle = "Full dataframe")
```
...
df_sample <- df_full %>% filter(date >= start & date <= end)
...
df_sample %>%
  ...
  ggthemes::theme_fivethirtyeight() +
  facet_wrap(~ site) +
  labs(title = "Number of articles/day", subtitle = "Full dataframe - filtered by Time")
```

## 2. select Medium

```{r}
keeps <- c("bild.de","focus.de","spiegel.de", "stern.de","handelsblatt.com", "welt.de", "zeit.de")
...
df_sample <- df_sample %>%
  ...
  mutate(medium = ifelse(site == "zeit.de","ZEIT ONLINE", site),
         ...
         medium = ifelse(site == "handelsblatt.com","Handelsblatt",medium)
         ) %>%
  select(-fullname) %>%
...
df_sample %>%
  ...
  ggthemes::theme_fivethirtyeight() +
  facet_wrap(~ site) +
  labs(title = "Number of articles/day", subtitle = "Full dataframe - filtered by Time & Medium")
```


## 3. select website section
...
```{r}
btw1 <- btw1 %>% filter(ressort2=="inland" | ressort2 =="deutschland" |  ressort2 =="bundestagswahl")
```
...
```{r}
btw <- btw1 %>%
  # remove articles with less than 120 words
...
clean.text <- function(x)
  ...
  # Handelsbblatt
  x = gsub("Sie haben den Artikel in Ihre Merkliste aufgenommen. Möchten Sie ihn nicht auch gleich mit Feunden teilen?", "", x, perl=TRUE)
  x = gsub("Alle Beiträge von dpa anzeige[^\n]*", "", x, perl = TRUE)
  x = gsub("Deutsche Presse-Agentur GmbH / Nachrichtenagentur", "", x, perl = TRUE)
...
dbWriteTable(con, "news_cleaned", 
             value = btw, overwrite = TRUE, row.names = FALSE)
```
