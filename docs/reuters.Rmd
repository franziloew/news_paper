...
Since 2012, the [Reuters Institute Digital News Survey](http://www.digitalnewsreport.org/about-us-2018/) has been investigating the media use of digital content. Among others, the following questions are investigated: What types of news are of interest? Which devices and media are used to find them?
...
Der Großteil der Befragten ordnet sich selbst weitgehend der politischen Mitte zu. Knapp 71 Prozent geben an, sich auf einer 7‐stufigen Skala genau in der Mitte oder leicht rechts bzw. leicht links wiederzufinden. Am rechten Rand sehen sich im Jahr 2018 6 Prozent der Befragten und 8 Prozent ordnen sich dem linken Rand zu. 
...
reutersDF2 %>%
  ...
  geom_text(vjust = 1) +
  labs(title = "Political orientation GER 2018",
       subtitle = "Reuters Institute Digital News Report 2018",
...
p1 <- reutersDF.long %>%
  filter(!grepl("know",medium)) %>%
  #filter(medium %in% keeps) %>%
  ggplot(aes(reorder(partisan.f, partisan_scale),reorder(medium, order_relative_m), fill = count_relative_m)) +
  geom_tile() +
