...
reutersDF1.grouped %>%
  ...
  coord_flip() +
  labs(x = NULL, y = NULL, title = "Use of online news (in the last week)",
...
#### News websites included in the model

```{r echo=FALSE, fig.height=6, fig.width=8, fig.align="centre"}
p1 <- reutersDF.long %>%
  filter(medium %in% keeps) %>%
  ggplot(aes(reorder(partisan.f, partisan_scale),
             reorder(medium, order_relative_m), 
...
```{r echo=FALSE, fig.height=8, fig.width=8}
reutersDF.long %>%
  filter(!grepl(paste("know","none",sep = "|"), medium, ignore.case = T)) %>%
  #filter(medium %in% keeps) %>%
  ggplot(aes(reorder(partisan.f, partisan_scale), 
