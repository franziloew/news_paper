...
```{r}
df %>% sample_n(10) %>% select(site, subsite, url, ressort1) %>% 
...
btw <- btw %>%
  ...
  filter(!grepl("deutschebahn", url, ignore.case = TRUE)) 
```
...
```{r eval=FALSE, include=FALSE}
processed <- textProcessor(btw$text_cleaned1, metadata = btw[,c("site","text_cleaned1")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
...
save(btw, out, df, file="../output/data_step2.Rda")
```
