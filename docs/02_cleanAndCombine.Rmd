...
```{r, include=FALSE}
# load the packages
libs <- c("tidytext","tidyr","readr","lubridate","tm","stm","RColorBrewer","RPostgreSQL","tidyverse",
          "plyr","class","knitr","data.table","SnowballC","parallel","gridExtra","jsonlite","stringr","scales")
...
```{sql connection=con}
select * from er_scrapes fetch first 5 rows only;
```
...
```{r}
# Modify the df so we can load it in Postgres 
df_new <- df %>%
  transmute(body = text,
            date = as.Date(date),
            isduplicate = NA,
            lang = "deu",
            shares = as.character(fb_shares),
            source = site,
            time = published,
            title = title,
            uri = NA,
...
```
```{r}
dbListFields(con, "er_scrapes")
```

```{r}
query <- tbl(con, 'er_scrapes')
```

```{r}
query %>%
  select(source) %>%
  mutate(site = substr(source,10,16) ) %>%
  show_query()
```

```{sql connection=con}
SELECT "source", substr("source", 10, 7) AS "site"
FROM (SELECT "source"
FROM "er_scrapes") "sbzsvknwwn"
```
...
clean.text <- function(x)
  ...
  # Bild.de
  x = gsub("Shop??bersicht Top Gutscheine", "", x)
  ...
  x = gsub('\\w{1,} f??r Abonnenten', "", x, perl = TRUE, ignore.case = TRUE)
  ...
  x = gsub('Erz??hlen Sie auf FOCUS Online ??ber Ihren Heimatort Teilen Sie Ihren Artikel und Ihr Foto', "", x, perl = TRUE)
  x = gsub("Bericht schreiben", "", x)
  x = gsub("Vielen Dank! Ihr Kommentar wurde abgeschickt.", "", x)
  x = gsub("Hier k??nnen Sie selbst Artikel verfassen:","", x)
  ...
  # x = gsub("Armbr??ster", "", x, ignore.case = TRUE, perl=TRUE)
  # x = gsub("M??nchenberg", "", x, ignore.case = TRUE, perl=TRUE)
  ...
  # x = gsub("Marc M??ller", "", x, ignore.case = TRUE, perl=TRUE)
...
# 2
mystopwords <- c("focus","online","spiegel", "stern", ".de", "bild","bildplus","n-tv.de", "zeit", "ersten","ard", "tagesschau","m??ssen","sagen","faktenfinder", "zeitmagazin","seitenanfang","ja","mal","heute","ich","sie","passwort","kommentar","wurde","ihr","der","im","artikel","mehr","ihren","foto","e","seien","comment","ticker","live","laif","uhr","videol??nge","dass","mindestens","das","mail","die","schon","neuer abschnitt", "login", "loggen", "inaktiv","nwmi","w??re","viele","nwnoa","morgenkolumne","beim","dpa","video","quelle","afp","witters","fotogalerie","wurden","worden","wegen","sagt","immer","gibt","geht","spon","registrierter","als","spiegel","vielen","in","es","bitte","dank","unserer","nutzer","sei","beitrag","user","seit","zeichen","tba","datenschutzerkl??rung","premium","nutzungsbedingungen","nutzungsrechte","pflichtfelder","registrierung","anzeige","gro??buchstaben","sonderzeichen","html","seitennavigation","fullscreen","statista","club","sagte","borenda","spreepicture","shop??bersicht","herr","imago","dobovisek","barenberg","heinlein","armbr??ster","kaess","m??nchenberg","b????sker","tsereteli","konietzny","klenkes","hauptstadtstudio","newsletter","premiumbereich","nachrichtenpodcast","karrierespiegel","picture alliance","appnutzer","civey","abo")
