...
load("../output/models/finalmodel_55.RDa")
...
newsLabels <- as.data.frame(sagelabs$cov.betas[[1]]$problabels) %>% 
  transmute(topic = as.numeric(rownames(.)),
            topic_name_news = paste("Topic",as.character(topic),":",V1,V2,V3,V4))

pressLabels <- as.data.frame(sagelabs$cov.betas[[2]]$problabels) %>% 
  transmute(topic = as.numeric(rownames(.)),
            topic_name_press = paste("Topic",as.character(topic),":",V1,V2,V3,V4))

topics.df <- left_join(newsLabels, pressLabels, by="topic")

topics.df %>% select(- topic) %>% htmlTable::htmlTable(align="l")
```


### Topic frequency

The expected proportion of the corpus that belongs to each topic is used to geht an initial overview of the results. The figure below displays the topics ordered by their expected frequency across the corpus. The four most frequent words in each topic are used as a label for that topic. 
...
overall_freq %>%
  ...
  labs(x=NULL, y=NULL, title="Expected Frequency of Topics", 
       subtitle="Most frequent words from the covariate 'news' is used to label the topics") 
```

```{r fig.height=10, fig.width=10}
overall_freq %>%
  ggplot(aes(reorder(topic_name_press, -order), frequency)) +
  geom_col(alpha = 0.7) +
  coord_flip() +
  ggthemes::theme_fivethirtyeight() +
  labs(x=NULL, y=NULL, title="Expected Frequency of Topics", 
       subtitle="Most frequent words from the covariate 'press release' is used to label the topics") 
```
...
correlations <- correlations %>% mutate(party = as.character(party), medium = as.character(medium))
```
...
ggplot(correlations, aes(party, cor)) +
  ...
  labs(x = NULL, y = NULL, title = "Topic correlation",
       subtitle = "Between news and press releases")
```
...
```{r}
ggplot(agendaBias, aes(party, agenda_bias_norm)) +
  geom_col() +
  coord_flip() +
  ggthemes::theme_fivethirtyeight() +
  facet_wrap(~medium) +
