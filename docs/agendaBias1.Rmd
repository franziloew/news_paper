...
save(model_df, out, stmOut, file = "../output/models/finalmodel_55.Rda")
```
...
ggsave("../figs/topic_proportion.png", height = 9)
```
...
ggsave("../figs/example_distr.png", height = 9, width = 8)
```
...
```{r eval=FALSE, include=FALSE}
topicmean_news %>%
  ggplot(aes(joint_label,topicmean)) +
  geom_col(fill="#0099c6", alpha = 0.8) +
  coord_flip() +
  theme_hc() +
  facet_grid(~source) +
...
```{r Function to estimate topic correlation}
...
correlations <- as.tibble(expand.grid(media$source, parties$source))
colnames(correlations) <- c("medium","party")

for (party in parties$source) {
  for (medium in media$source) {
...
ggsave("../figs/agenda_correlation.png", width = 7, height = 3)
```
...
```{r eval=FALSE, include=FALSE}
agendaBias %>%
  ggplot(aes(medium, agenda_bias_norm)) +
  geom_col(fill="#0099c6", alpha=0.8) +
  theme_hc() +
  facet_grid(~party) +
  labs(x = NULL, y = "correlation") +
...
ggsave("../figs/agenda_bias.png", width = 7, height = 3)
```
