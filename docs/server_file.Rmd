...
# TRUE
save(news_df, file = "../output/news_df.Rda")
```
...
plotTopics <- function(x){
  sent_df %>%
    ...
    ggplot(aes(reorder(source, dummy_theta), dummy_theta,fill=dummy_theta)) +
...
```{r}
corr.df <- theta %>%
  group_by(topic, type, source, topic_label) %>%
  dplyr::summarise(
...
corr.df.sm <-sent_df %>%
  # filter documents that are not about a party
  filter(ident != 0) %>%
  group_by(topic, source) %>%
  summarise(dummy_theta = mean(dummy_theta)) %>%
  ungroup() %>%
  spread(source, dummy_theta)

x <- as.matrix(corr.df.sm[,-1])

rs <- corrr::correlate(x)
