...
wordcloud::wordcloud(model_df$title_text, max.words = 400)
...
wordcloud::wordcloud(model_df$text_cleaned1, max.words = 400)
...
```{r eval=FALSE, include=FALSE}
tokens <- model_df %>% unnest_tokens(word, text_cleaned1)

tokens.count <- tokens %>%
  count(source, type, word, sort = TRUE) %>%
  ungroup() %>%
...
tokens.count %>% 
  ...
  geom_col(fill="#5dc863", show.legend = FALSE) +
  theme_hc() +
  labs(x = NULL, y = NULL) +
  facet_wrap(~source, ncol = 3, scales = "free") +
  coord_flip()

ggsave("../figs/tokens_news.png", width = 12, height = 12)
```

![](../figs/tokens_news.png)
...
tokens.count %>% 
  ...
  ungroup() %>%
  ggplot(aes(word, tf_idf)) +
  geom_col(fill="#440154", show.legend = FALSE) +
  theme_hc() +
  labs(x = NULL, y = NULL) +
  facet_wrap(~source, ncol = 2, scales = "free") +
  coord_flip()

ggsave("../figs/tokens_press.png", width = 12, height = 12)
```

![](../figs/tokens_press.png)
...
save(model_df, out, stmOut, file = "../output/models/finalmodel_60_nocontet.RDa")
```
