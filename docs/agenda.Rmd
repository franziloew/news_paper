...
- Pressemitteilungen bilden nur einen Weg in die Öffentlichkeit. Bei einer umfassenden Betrachtung müsste man im Kontext auch Interviews, Veranstaltungen, Hintergrundgespräche, etc. betrachten. -> zu umfangreich
...
processed <- textProcessor(model_df$text_cleaned, 
                           metadata = model_df[,c("source","type","text_cleaned")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
...
rm(list = ls())
load("../output/models/finalmodel_50_nocontet.RDa")
...
theta %>%
  ...
  filter(topic == 14) %>%
  arrange(desc(theta)) %>%
  select(source, theta, title_text) 
```

```{r}
sagelabs$marginal$prob[14,]
```
