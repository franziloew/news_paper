...
```{r}
# load data
wahlumfragen <- read.csv("../data/wahlumfragen.csv", sep=";", stringsAsFactors=FALSE)

# get Mittelwert variables
keeps <- grep("Mittelwert", colnames(wahlumfragen))

# reduce df
polls <- wahlumfragen[,keeps]

# rename cols
names <- stringr::str_replace_all(colnames(wahlumfragen[,keeps]),pattern = ".Gleitender.Mittelwert","")
names(polls) <- c(names)

# add date variables
polls <- polls %>%
  mutate(date = as.Date(wahlumfragen$Datum,format = "%d.%m.%y"))
```

```{r}
color = c("#1298d3","#2e1b40","#fbca5b","#4fa041","#bd3075","#d11f2f")

polls %>%
  gather("party", "value", AfD:SPD) %>%
