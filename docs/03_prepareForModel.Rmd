...
```{r}
load(file="../output/btw_cleaned.Rda")
```
...
bigrams <- btw %>%
  ...
  # separate words to match it with the sentiment df
  separate(bigram, c("word1", "word"), sep = " ")

# Combine second word with sentiment values
sentDF2 <- left_join(bigrams, sent, by="word") %>%
  ...
  # delete value where word1 is "nicht" 
  filter(!grepl("nicht",word1,ignore.case = F))
```
...
df <- df %>% 
  ...
  # calculate sentiment
  mutate(
    sent_diff_d = sum_positive_d - abs(sum_negative_d),
    sent_diff = sum_positive - abs(sum_negative))

df %>%
  sample_n(10) %>%
  select(sum_positive,sum_negative, sent_diff,
         sum_positive_d, sum_negative_d, sent_diff_d)
```
...
df %>%
  mutate(sentiment = normalize_data(sentiment)) %>%
  filter(sentiment < .1 ) %>%
  select(articleID, title_text, sentiment) %>%
  htmlTable::htmlTable()
```

```{r}
sentDF %>%
  filter(articleID == 9932) %>%
  select(word, negative, positive, negative_d, positive_d)
```
...
save(df, out, file="../output/btw_out2.Rda")
```
...
```{r}
library(stm)
stmOut <- stm(documents = out$documents, vocab = out$vocab, K = 40, prevalence = ~site, 
    data = out$meta, init.type = "Spectral", 
    seed = 69874, max.em.its = 75)

save(df,out,stmOut, file = "../output/models/stm40.Rda")
```
