...
pressReleases <- bind_rows(cdu, spd, afd, gruene, linke, fdp) %>%
  ...
  mutate(
    ...
    filtered = ifelse(text_length < 50, TRUE, filtered)
  )
```
...
pressReleases %>%
  ...
  coord_flip() +
  theme_hc() +
  scale_fill_viridis_d(labels = c("Party","Parliamentary group")) +
...
```{r}
pressReleases %>%
  filter(text_length < 1500) %>%
  ggplot(aes(x=party, y=text_length)) +
  geom_boxplot() +
  theme_hc() +
  #stat_summary(fun.y=mean, geom="point", shape=20, size=3, color="red", fill="red") +
  labs(x=NULL, y="# of words", caption = "37 articles with a text length > 1500 words are not plotted:\nFDP: 37")
