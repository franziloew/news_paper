...
- Get a sentiment value for each document (press and news).
- get a distribution over topics for each document (press and news).
- how does each party/medium talk about each topic (e.g. what is the sentiment score?)
- estimate the correlation matrix based on the topic-polarity vector (multiplied by the topic probability?)
...
sent_df <- prep_df %>%
    dplyr::mutate(split = get_sentences(title_text)) %$%
    sentiment(split)
